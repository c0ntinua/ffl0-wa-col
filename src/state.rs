...
pub fn display_color(red_state : &State,green_state : &State,blue_state : &State, rows : usize, cols : usize, right : usize, down : usize) -> () {
    for row in 0..rows {
        cursor_to(row + down + 1, right);
        for col in 0..cols {
            set_color(hue(red_state[row][col]), hue(green_state[row][col]),hue(blue_state[row][col]));
            print!("\u{2588}");
        }
    }  
}
